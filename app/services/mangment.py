...
def get_classfied_social_networks(query, fromDate, toDate, subreddit, post_limit, comment_limit): 
    try:
        ...
        # Format dates 
        toDate = datetime.strptime(toDate, '%Y-%m-%d').strftime('%Y/%m/%d') if toDate != '' else None
        fromDate = datetime.strptime(fromDate, '%Y-%m-%d').strftime('%Y/%m/%d') if fromDate != '' else None
